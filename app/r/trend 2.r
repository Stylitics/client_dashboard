...
# and so the connection(s) across this script to become:
# con <- dbConnect(drv, host=host, port=port, dbname=dbname, user=user, password=password)

# other stuff that will share here:
# param1 <- {{param1}}
# param2 <- {{param2}}
# param3 <- {{param3}}
...
population$sortOpt <- "User wornItemsCountDF"
population$studentOpt <- "All"
population$styleTxt <- ""
population$colorTxt <- ""
population$brandTxt <- ""
